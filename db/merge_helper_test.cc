...
TEST_F(MergeHelperTest, MergeAtBottomSuccess) {
  ...
  AddKeyVal("a", 20, ValueType::kTypeMerge, test::EncodeInt(1U));
  AddKeyVal("a", 10, ValueType::kTypeMerge, test::EncodeInt(3U));
  AddKeyVal("b", 10, ValueType::kTypeMerge, test::EncodeInt(4U));  // <- iter_ after merge

  ASSERT_TRUE(Run(0, true).ok());
  ASSERT_EQ(ks_[2], iter_->key());
  ASSERT_EQ(test::KeyStr("a", 20, ValueType::kTypeValue), merge_helper_->keys()[0]);
...
TEST_F(MergeHelperTest, MergeValue) {
  ...
  AddKeyVal("a", 40, ValueType::kTypeMerge, test::EncodeInt(1U));
  AddKeyVal("a", 30, ValueType::kTypeMerge, test::EncodeInt(3U));
  AddKeyVal("a", 20, ValueType::kTypeValue, test::EncodeInt(4U));  // <- iter_ after merge
  AddKeyVal("a", 10, ValueType::kTypeMerge, test::EncodeInt(1U));

  ASSERT_TRUE(Run(0, false).ok());
  ASSERT_EQ(ks_[3], iter_->key());
  ASSERT_EQ(test::KeyStr("a", 40, ValueType::kTypeValue), merge_helper_->keys()[0]);
...
TEST_F(MergeHelperTest, SnapshotBeforeValue) {
  ...
  AddKeyVal("a", 50, ValueType::kTypeMerge, test::EncodeInt(1U));
  AddKeyVal("a", 40, ValueType::kTypeMerge, test::EncodeInt(3U));  // <- iter_ after merge
  AddKeyVal("a", 30, ValueType::kTypeMerge, test::EncodeInt(1U));
  AddKeyVal("a", 20, ValueType::kTypeValue, test::EncodeInt(4U));
  AddKeyVal("a", 10, ValueType::kTypeMerge, test::EncodeInt(1U));

  ASSERT_TRUE(Run(31, true).IsMergeInProgress());
  ASSERT_EQ(ks_[2], iter_->key());
  ASSERT_EQ(test::KeyStr("a", 50, ValueType::kTypeMerge), merge_helper_->keys()[0]);
  ASSERT_EQ(test::EncodeInt(4U), merge_helper_->values()[0]);
  ASSERT_EQ(1U, merge_helper_->keys().size());
  ASSERT_EQ(1U, merge_helper_->values().size());
}
...
TEST_F(MergeHelperTest, NoPartialMerge) {
  ...
  AddKeyVal("a", 50, ValueType::kTypeMerge, "v2");
  AddKeyVal("a", 40, ValueType::kTypeMerge, "v");  // <- iter_ after merge
  AddKeyVal("a", 30, ValueType::kTypeMerge, "v");

  ASSERT_TRUE(Run(31, true).IsMergeInProgress());
  ASSERT_EQ(ks_[2], iter_->key());
  ASSERT_EQ(test::KeyStr("a", 40, ValueType::kTypeMerge), merge_helper_->keys()[0]);
  ASSERT_EQ("v", merge_helper_->values()[0]);
  ASSERT_EQ(test::KeyStr("a", 50, ValueType::kTypeMerge), merge_helper_->keys()[1]);
...
TEST_F(MergeHelperTest, MergeDeletion) {
  ...
  AddKeyVal("a", 30, ValueType::kTypeMerge, test::EncodeInt(3U));
  AddKeyVal("a", 20, ValueType::kTypeDeletion, "");

  ASSERT_TRUE(Run(15, false).ok());
  ASSERT_FALSE(iter_->Valid());
  ASSERT_EQ(test::KeyStr("a", 30, ValueType::kTypeValue), merge_helper_->keys()[0]);
...
TEST_F(MergeHelperTest, CorruptKey) {
  ...
  AddKeyVal("a", 30, ValueType::kTypeMerge, test::EncodeInt(3U));
  AddKeyVal("a", 25, ValueType::kTypeMerge, test::EncodeInt(1U));
  // Corrupt key
  AddKeyVal("a", 20, ValueType::kTypeDeletion, "", true);  // <- iter_ after merge

  ASSERT_TRUE(Run(15, false).IsMergeInProgress());
  ASSERT_EQ(ks_[2], iter_->key());
  ASSERT_EQ(test::KeyStr("a", 30, ValueType::kTypeMerge), merge_helper_->keys()[0]);
  ASSERT_EQ(test::EncodeInt(4U), merge_helper_->values()[0]);
  ASSERT_EQ(1U, merge_helper_->keys().size());
  ASSERT_EQ(1U, merge_helper_->values().size());
}
...
TEST_F(MergeHelperTest, FilterMergeOperands) {
  ...
  AddKeyVal("a", 30, ValueType::kTypeMerge, test::EncodeInt(3U));
  AddKeyVal("a", 29, ValueType::kTypeMerge, test::EncodeInt(5U));  // Filtered
  AddKeyVal("a", 28, ValueType::kTypeMerge, test::EncodeInt(3U));
  AddKeyVal("a", 27, ValueType::kTypeMerge, test::EncodeInt(1U));
  AddKeyVal("a", 26, ValueType::kTypeMerge, test::EncodeInt(5U));  // Filtered
  AddKeyVal("a", 25, ValueType::kTypeValue, test::EncodeInt(1U));

  ASSERT_TRUE(Run(15, false).ok());
  ASSERT_FALSE(iter_->Valid());
  MergeOutputIterator merge_output_iter(merge_helper_.get());
  merge_output_iter.SeekToFirst();
  ASSERT_EQ(test::KeyStr("a", 30, ValueType::kTypeValue),
...
TEST_F(MergeHelperTest, FilterAllMergeOperands) {
  ...
  AddKeyVal("a", 30, ValueType::kTypeMerge, test::EncodeInt(5U));
  AddKeyVal("a", 29, ValueType::kTypeMerge, test::EncodeInt(5U));
  AddKeyVal("a", 28, ValueType::kTypeMerge, test::EncodeInt(5U));
  AddKeyVal("a", 27, ValueType::kTypeMerge, test::EncodeInt(5U));
  AddKeyVal("a", 26, ValueType::kTypeMerge, test::EncodeInt(5U));
  AddKeyVal("a", 25, ValueType::kTypeMerge, test::EncodeInt(5U));
  ...
  AddKeyVal("a", 24, ValueType::kTypeDeletion, test::EncodeInt(5U));
  AddKeyVal("b", 23, ValueType::kTypeValue, test::EncodeInt(5U));
  ...
  ASSERT_EQ(test::KeyStr("a", 24, ValueType::kTypeDeletion), iter_->key().ToString());
...
TEST_F(MergeHelperTest, FilterFirstMergeOperand) {
  ...
  AddKeyVal("a", 31, ValueType::kTypeMerge, test::EncodeInt(5U));  // Filtered
  AddKeyVal("a", 30, ValueType::kTypeMerge, test::EncodeInt(5U));  // Filtered
  AddKeyVal("a", 29, ValueType::kTypeMerge, test::EncodeInt(2U));
  AddKeyVal("a", 28, ValueType::kTypeMerge, test::EncodeInt(1U));
  AddKeyVal("a", 27, ValueType::kTypeMerge, test::EncodeInt(3U));
  AddKeyVal("a", 26, ValueType::kTypeMerge, test::EncodeInt(5U));  // Filtered
  AddKeyVal("a", 25, ValueType::kTypeMerge, test::EncodeInt(5U));  // Filtered
  AddKeyVal("b", 24, ValueType::kTypeValue, test::EncodeInt(5U));  // next user key
  ...
  // out
  ASSERT_EQ(test::KeyStr("a", 29, ValueType::kTypeValue),
            merge_output_iter.key().ToString());
...
TEST_F(MergeHelperTest, DontFilterMergeOperandsBeforeSnapshotTest) {
  ...
  AddKeyVal("a", 31, ValueType::kTypeMerge, test::EncodeInt(5U));
  AddKeyVal("a", 30, ValueType::kTypeMerge, test::EncodeInt(5U));
  AddKeyVal("a", 29, ValueType::kTypeMerge, test::EncodeInt(2U));
  AddKeyVal("a", 28, ValueType::kTypeMerge, test::EncodeInt(1U));
  AddKeyVal("a", 27, ValueType::kTypeMerge, test::EncodeInt(3U));
  AddKeyVal("a", 26, ValueType::kTypeMerge, test::EncodeInt(5U));
  AddKeyVal("a", 25, ValueType::kTypeMerge, test::EncodeInt(5U));
  AddKeyVal("b", 24, ValueType::kTypeValue, test::EncodeInt(5U));

  ASSERT_OK(Run(15, true, 32));
  ASSERT_TRUE(iter_->Valid());
  MergeOutputIterator merge_output_iter(merge_helper_.get());
  merge_output_iter.SeekToFirst();
  ASSERT_EQ(test::KeyStr("a", 31, ValueType::kTypeValue),
            merge_output_iter.key().ToString());
