 ...
 public:
  ...
  void SetupOptions(const rs::advanced_options::CompactionStyle compaciton_style,
    ...
    if (compaciton_style == rs::advanced_options::CompactionStyle::Level) {
      ...
      rs::universal_compaction::CompactionOptionsUniversal universal_options = rs::universal_compaction::CompactionOptionsUniversal_new();
      ...
      rs::advanced_options::CompactionOptionsFIFO fifo_options = rs::advanced_options::CompactionOptionsFIFO_new();
...
TEST_F(DBCompactionTest, ChangeLevelCompactRangeConflictsWithManual) {
  ...
  rocksdb::port::Thread refit_level_thread([&] {
    CompactRangeOptions cro;
    cro.change_level = true;
    cro.target_level = 1;
    ASSERT_OK(dbfull()->CompactRange(cro, nullptr, nullptr));
...
TEST_F(DBCompactionTest, CompactionWithChecksumHandoff1) {
  ...
  ASSERT_EQ(s.severity(),
            rocksdb::Status::Severity::kUnrecoverableError);
  SyncPoint::GetInstance()->DisableProcessing();
  Destroy(options);
  Reopen(options);
...
TEST_F(DBCompactionTest, CompactionWithChecksumHandoffManifest1) {
  ...
  ASSERT_EQ(s, Status::OK());
  s = dbfull()->TEST_WaitForCompact();
  ASSERT_EQ(s.severity(), rocksdb::Status::Severity::kFatalError);
  SyncPoint::GetInstance()->DisableProcessing();
  Destroy(options);
}
