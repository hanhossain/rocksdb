...
TEST_F(DBBasicTestWithTimestamp, SimpleIterate) {
  ...
  for (size_t i = 0; i < read_timestamps.size(); ++i) {
    ...
    for (it->Seek(Key1(0)), key = start_keys[i]; it->Valid();
         it->Next(), ++count, ++key) {
      CheckIterUserEntry(it.get(), Key1(key), ValueType::kTypeValue,
    ...
    while (l < r) {
      ...
      for (it->SeekToLast(), key = std::min(r, kMaxKey + 1), count = 0;
        ...
        CheckIterUserEntry(it.get(), Key1(key - 1), ValueType::kTypeValue,
                           "value" + std::to_string(i), write_timestamps[i]);
      }
      l += (kMaxKey / 100);
      r -= (kMaxKey / 100);
...
TEST_P(DBBasicTestWithTimestampTableOptions, SeekWithBound) {
  ...
  read_opts.auto_prefix_mode = true;
  {
    ...
    // may return invalid result.
    iter->Seek("foo");
    CheckIterUserEntry(iter.get(), lo_bound, ValueType::kTypeValue, "bar2",
                       Timestamp(1, 0));
    iter->SeekToFirst();
    CheckIterUserEntry(iter.get(), lo_bound, ValueType::kTypeValue, "bar2",
                       Timestamp(1, 0));
    iter->SeekForPrev("g");
    CheckIterUserEntry(iter.get(), "foo4", ValueType::kTypeValue, "bar4", Timestamp(1, 0));
    iter->SeekToLast();
    CheckIterUserEntry(iter.get(), "foo4", ValueType::kTypeValue, "bar4", Timestamp(1, 0));
  }
...
TEST_F(DBBasicTestWithTimestamp, ChangeIterationDirection) {
  ...
  CheckIterUserEntry(it.get(), std::get<0>(kvs[0]), ValueType::kTypeValue,
                     std::get<1>(kvs[0]), Timestamp(4, 3));
  it->Next();
  CheckIterUserEntry(it.get(), std::get<0>(kvs[1]), ValueType::kTypeValue,
...
TEST_F(DBBasicTestWithTimestamp, SimpleForwardIterateLowerTsBound) {
  ...
  for (size_t i = 0; i < read_timestamps.size(); ++i) {
    ...
    for (it->Seek(Key1(0)), key = 0; it->Valid(); it->Next(), ++count, ++key) {
      CheckIterEntry(it.get(), Key1(key), ValueType::kTypeValue,
                     "value" + std::to_string(i), write_timestamps[i]);
      if (i > 0) {
        it->Next();
        CheckIterEntry(it.get(), Key1(key), ValueType::kTypeValue,
  ...
  // Delete all keys@ts=5 and check iteration result with start ts set
  {
    ...
    uint64_t key = 0;
    for (it->Seek(Key1(0)), key = 0; it->Valid(); it->Next(), ++count, ++key) {
      CheckIterEntry(it.get(), Key1(key), ValueType::kTypeDeletionWithTimestamp, Slice(),
                     write_timestamp);
...
TEST_F(DBBasicTestWithTimestamp, BackwardIterateLowerTsBound) {
  ...
  for (size_t i = 0; i < read_timestamps.size(); ++i) {
    ...
    for (it->SeekForPrev(Key1(kMaxKey)), key = kMaxKey; it->Valid();
         it->Prev(), ++count, --key) {
      CheckIterEntry(it.get(), Key1(key), ValueType::kTypeValue, "value0",
                     write_timestamps[0]);
      if (i > 0) {
        it->Prev();
        CheckIterEntry(it.get(), Key1(key), ValueType::kTypeValue, "value1",
                       write_timestamps[1]);
      }
    }
    size_t expected_count = kMaxKey + 1;
...
TEST_F(DBBasicTestWithTimestamp, SimpleBackwardIterateLowerTsBound) {
...

  {
    ...
    std::unique_ptr<Iterator> it(db_->NewIterator(read_opts));
    it->SeekToLast();
    CheckIterEntry(it.get(), "a", ValueType::kTypeSingleDeletion, Slice(),
                   Timestamp(1, 0));
...
TEST_F(DBBasicTestWithTimestamp, BackwardIterateLowerTsBound_Reseek) {
...

  {
    ...
    for (int i = 0; i < 3 && it->Valid(); it->Prev(), ++i) {
      CheckIterEntry(it.get(), "b", ValueType::kTypeValue, "v" + std::to_string(4 + i),
                     Timestamp(4 + i, 0));
    }
    for (int i = 0; i < 3 && it->Valid(); it->Prev(), ++i) {
      CheckIterEntry(it.get(), "a", ValueType::kTypeValue, "v" + std::to_string(4 + i),
                     Timestamp(4 + i, 0));
    }
  }
...
TEST_F(DBBasicTestWithTimestamp, ReseekToTargetTimestamp) {
  ...
  }
  {
    ...
    std::unique_ptr<Iterator> iter(db_->NewIterator(read_opts));
    iter->SeekToFirst();
    CheckIterUserEntry(iter.get(), "foo", ValueType::kTypeValue, "value0", ts_str);
    ASSERT_EQ(
    ...
    iter->SeekToLast();
    CheckIterUserEntry(iter.get(), "foo", ValueType::kTypeValue,
...
TEST_F(DBBasicTestWithTimestamp, ReseekToNextUserKey) {
  ...
  }
  {
    ...
    iter->Seek("a");
    iter->Next();
    CheckIterUserEntry(iter.get(), "b", ValueType::kTypeValue, "new_value", ts_str);
    ASSERT_EQ(
        1, options.statistics->getTickerCount(NUMBER_OF_RESEEKS_IN_ITERATION));
  }
  Close();
...
TEST_F(DBBasicTestWithTimestamp, ReseekToUserKeyBeforeSavedKey) {
  ...
  }
  {
    ...
    iter->SeekToLast();
    iter->Prev();
    CheckIterUserEntry(iter.get(), "a", ValueType::kTypeValue, "value", ts_str);
    ASSERT_EQ(
        1, options.statistics->getTickerCount(NUMBER_OF_RESEEKS_IN_ITERATION));
  }
  Close();
...
// Two L0s:
//       f1                                  f2
// <a, 1, ValueType::kTypeValue>    <a, 3, ValueType::kTypeDeletionWithTimestamp>...<b, 2, ValueType::kTypeValue>
...
TEST_P(DBBasicTestWithTimestampPrefixSeek, IterateWithPrefix) {
                                                 ...
                                                 Timestamp(9, 0xffffffff)};
  {
    ...
    for (size_t i = 0; i != read_ts_list.size(); ++i) {
      ...
      CheckIterUserEntry(iter.get(), Key1(kMaxKey), ValueType::kTypeValue,
      ...
      CheckIterUserEntry(iter.get(), Key1(kMinKey), ValueType::kTypeValue,
                         "value" + std::to_string(i), write_ts_list[i]);
      iter->Prev();
      ASSERT_FALSE(iter->Valid());
    }
    ...
    for (size_t i = 0; i != read_ts_list.size(); ++i) {
      ...
      for (size_t j = 0; j != targets.size(); ++j) {
        ...
        while (it->Valid()) {
          ...
          CheckIterUserEntry(it.get(), Key1(expected_key), ValueType::kTypeValue,
                             "value" + std::to_string(i), write_ts_list[i]);
          ++count;
          ++expected_key;
          it->Next();
