...
TEST_F(DBMemTableTest, DuplicateSeq) {
  ...
  ASSERT_OK(
      mem->Add(seq, rs::db::dbformat::ValueType::kTypeValue, "key", "value2", nullptr /* kv_prot_info */));
  ASSERT_TRUE(
      mem->Add(seq, rs::db::dbformat::ValueType::kTypeValue, "key", "value2", nullptr /* kv_prot_info */)
          .IsTryAgain());
  // Changing the type should still cause the duplicatae key
  ASSERT_TRUE(
      mem->Add(seq, rs::db::dbformat::ValueType::kTypeMerge, "key", "value2", nullptr /* kv_prot_info */)
          .IsTryAgain());
  // Changing the seq number will make the key fresh
  ASSERT_OK(mem->Add(seq + 1, rs::db::dbformat::ValueType::kTypeMerge, "key", "value2",
                     nullptr /* kv_prot_info */));
  // Test with different types for duplicate keys
  ASSERT_TRUE(
      mem->Add(seq, rs::db::dbformat::ValueType::kTypeDeletion, "key", "", nullptr /* kv_prot_info */)
          .IsTryAgain());
  ASSERT_TRUE(
      mem->Add(seq, rs::db::dbformat::ValueType::kTypeSingleDeletion, "key", "", nullptr /* kv_prot_info */)
          .IsTryAgain());
  ...
  for (int i = 0; i < 10000; i++) {
    ...
    Status s = mem->Add(seq, rs::db::dbformat::ValueType::kTypeValue, "foo", "value" + std::to_string(seq),
                        nullptr /* kv_prot_info */);
    if (insert_dup) {
  ...
  ASSERT_OK(
      mem->Add(seq, rs::db::dbformat::ValueType::kTypeValue, "key_1", "value", nullptr /* kv_prot_info */));
  ASSERT_TRUE(
      mem->Add(seq, rs::db::dbformat::ValueType::kTypeValue, "key_1", "value", nullptr /* kv_prot_info */)
          .IsTryAgain());
  delete mem;
  ...
  ASSERT_OK(mem->Add(seq, rs::db::dbformat::ValueType::kTypeValue, "key", "value",
                     nullptr /* kv_prot_info */, true, &post_process_info));
  ASSERT_TRUE(mem->Add(seq, rs::db::dbformat::ValueType::kTypeValue, "key", "value",
                       nullptr /* kv_prot_info */, true, &post_process_info)
                  .IsTryAgain());
  delete mem;
}
...
TEST_F(DBMemTableTest, ConcurrentMergeWrite) {
  ...
  ASSERT_OK(mem->Add(0, rs::db::dbformat::ValueType::kTypeValue, "key", value, nullptr /* kv_prot_info */));
  value.clear();
  ...
  ROCKSDB_NAMESPACE::port::Thread write_thread1([&]() {
    ...
    for (int seq = 1; seq < num_ops / 2; seq++) {
      PutFixed64(&v1, seq);
      ASSERT_OK(mem->Add(seq, rs::db::dbformat::ValueType::kTypeMerge, "key", v1, nullptr /* kv_prot_info */,
  ...
  ROCKSDB_NAMESPACE::port::Thread write_thread2([&]() {
    ...
    for (int seq = num_ops / 2; seq < num_ops; seq++) {
      PutFixed64(&v2, seq);
      ASSERT_OK(mem->Add(seq, rs::db::dbformat::ValueType::kTypeMerge, "key", v2, nullptr /* kv_prot_info */,
