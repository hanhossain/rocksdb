...
TEST_F(VersionEditTest, EncodeDecode) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    edit.AddFile(3, kBig + 300 + i, kBig32Bit + 400 + i, 0,
                 InternalKey("foo", kBig + 500 + i, rs::db::dbformat::ValueType::TypeValue),
                 InternalKey("zoo", kBig + 600 + i, rs::db::dbformat::ValueType::TypeDeletion),
...
TEST_F(VersionEditTest, EncodeDecodeNewFile4) {
  ...
  VersionEdit edit;
  edit.AddFile(3, 300, 3, 100, InternalKey("foo", kBig + 500, rs::db::dbformat::ValueType::TypeValue),
               InternalKey("zoo", kBig + 600, rs::db::dbformat::ValueType::TypeDeletion), kBig + 500,
  ...
  edit.AddFile(4, 301, 3, 100, InternalKey("foo", kBig + 501, rs::db::dbformat::ValueType::TypeValue),
               InternalKey("zoo", kBig + 601, rs::db::dbformat::ValueType::TypeDeletion), kBig + 501,
               kBig + 601, false, rs::advanced_options::Temperature::Unknown, kInvalidBlobFileNumber,
               kUnknownOldestAncesterTime, kUnknownFileCreationTime,
               301 /* epoch_number */, kUnknownFileChecksum,
               kUnknownFileChecksumFuncName, kNullUniqueId64x2, 0);
  edit.AddFile(5, 302, 0, 100, InternalKey("foo", kBig + 502, rs::db::dbformat::ValueType::TypeValue),
               InternalKey("zoo", kBig + 602, rs::db::dbformat::ValueType::TypeDeletion), kBig + 502,
               ...
               kUnknownFileChecksumFuncName, kNullUniqueId64x2, 0);
  edit.AddFile(5, 303, 0, 100, InternalKey("foo", kBig + 503, rs::db::dbformat::ValueType::TypeBlobIndex),
               InternalKey("zoo", kBig + 603, rs::db::dbformat::ValueType::TypeBlobIndex), kBig + 503,
...
TEST(FileMetaDataTest, UpdateBoundariesBlobIndex) {
  ...
  // Plain old value (does not affect oldest_blob_file_number)
  {
    ...
    ASSERT_OK(meta.UpdateBoundaries(key, value, seq, rs::db::dbformat::ValueType::TypeValue));
  ...
  // Non-inlined, non-TTL blob index (sets oldest_blob_file_number)
  {
    ...
    ASSERT_OK(meta.UpdateBoundaries(key, blob_index, seq, rs::db::dbformat::ValueType::TypeBlobIndex));
  ...
  // Corrupt blob index
  {
    ...
    ASSERT_TRUE(
        meta.UpdateBoundaries(key, corrupt_blob_index, seq, rs::db::dbformat::ValueType::TypeBlobIndex)
            .IsCorruption());
    ASSERT_EQ(meta.oldest_blob_file_number, expected_oldest_blob_file_number);
  }
  ...
  // Invalid blob file number
  {
    ...
    ASSERT_TRUE(meta.UpdateBoundaries(key, blob_index, seq, rs::db::dbformat::ValueType::TypeBlobIndex)
                    .IsCorruption());
    ASSERT_EQ(meta.oldest_blob_file_number, expected_oldest_blob_file_number);
  }
}
