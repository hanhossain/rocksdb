...
Status DBImpl::ResumeImpl(DBRecoverContext context) {
  ...
  if (s.ok()) {
    ...
    if (bg_error.severity() > rs::status::Severity::kHardError) {
      ROCKS_LOG_INFO(
...
InternalIterator* DBImpl::NewInternalIterator(
  ...
  if (s.ok()) {
    ...
    if (read_options.read_tier != rs::options::ReadTier::MemtableTier) {
...
Status DBImpl::GetImpl(const ReadOptions& read_options, const Slice& key,
  ...
  bool skip_memtable = (read_options.read_tier == rs::options::ReadTier::PersistedTier &&
                        has_unpersisted_data_.load(std::memory_order_relaxed));
  bool done = false;
        ...
        *get_impl_options.number_of_operands =
        ...
        if (*get_impl_options.number_of_operands >
            get_impl_options.get_merge_operands_options
                ->expected_max_number_of_operands) {
          s = Status::Incomplete(
              rs::status::SubCode::KMergeOperandsInsufficientCapacity);
        } else {
...
std::vector<Status> DBImpl::MultiGet(
  ...
  for (keys_read = 0; keys_read < num_keys; ++keys_read) {
    ...
    bool skip_memtable =
        (read_options.read_tier == rs::options::ReadTier::PersistedTier &&
         has_unpersisted_data_.load(std::memory_order_relaxed));
    bool done = false;
    if (!skip_memtable) {
...
bool DBImpl::KeyMayExist(const ReadOptions& read_options,
  ...
  roptions.read_tier = rs::options::ReadTier::BlockCacheTier;  // read from block cache only
  PinnableSlice pinnable_val;
  GetImplOptions get_impl_options;
  get_impl_options.column_family = column_family;
