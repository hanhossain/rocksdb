...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  if (status.ok()) {
    ...
    stats->AddDBStats(InternalStats::InternalDBStatsType::kIntStatsNumKeysWritten, total_count,
                      concurrent_update);
    RecordTick(stats_, NUMBER_KEYS_WRITTEN, total_count);
    stats->AddDBStats(InternalStats::InternalDBStatsType::kIntStatsBytesWritten, total_byte_size,
                      concurrent_update);
    RecordTick(stats_, BYTES_WRITTEN, total_byte_size);
    stats->AddDBStats(InternalStats::InternalDBStatsType::kIntStatsWriteDoneBySelf, 1,
                      concurrent_update);
    RecordTick(stats_, WRITE_DONE_BY_SELF);
    auto write_done_by_other = write_group.size - 1;
    if (write_done_by_other > 0) {
      stats->AddDBStats(InternalStats::InternalDBStatsType::kIntStatsWriteDoneByOther,
...
Status DBImpl::PreprocessWrite(const WriteOptions& write_options,
  ...
  if (UNLIKELY(status.ok() && write_buffer_manager_->ShouldStall())) {
    default_cf_internal_stats_->AddDBStats(
        InternalStats::InternalDBStatsType::kIntStatsWriteBufferManagerLimitStopsCounts, 1,
        true /* concurrent */);
...
IOStatus DBImpl::WriteToWAL(const WriteThread::WriteGroup& write_group,
  ...
  if (io_s.ok()) {
    ...
    stats->AddDBStats(InternalStats::InternalDBStatsType::kIntStatsWriteWithWal, write_with_wal);
...
Status DBImpl::DelayWrite(uint64_t num_bytes, WriteThread& write_thread,
  ...
  if (delayed) {
    default_cf_internal_stats_->AddDBStats(
        InternalStats::InternalDBStatsType::kIntStatsWriteStallMicros, time_delayed);
