...
TEST_F(DBTest, LevelReopenWithFIFO) {
  ...
  for (int i = 0; i < kTotalSstFileCount; ++i) {
    ...
    fifo_options.compaction_style = rs::advanced_options::CompactionStyle::FIFO;
    options.create_if_missing = false;
...
void CheckColumnFamilyMeta(
  ...
  for (size_t i = 0; i < cf_meta.levels.size(); ++i) {
    ...
    for (size_t j = 0; j < level_meta_from_cf.files.size(); ++j) {
      ...
      ASSERT_EQ(file_meta_from_cf.file_type, rs::types::FileType::TableFile);
...
void CheckLiveFilesMeta(
  ...
  for (const auto& meta : live_file_meta) {
    ...
    // More from FileStorageInfo
    ASSERT_EQ(meta.file_type, rs::types::FileType::TableFile);
...
TEST_F(DBTest, SnapshotFiles) {
  do {
    ...
    for (auto& info : new_infos) {
      ...
      if (info.file_type == rs::types::FileType::DescriptorFile) {
...
TEST_F(DBTest, DynamicUniversalCompactionOptions) {
  ...
  ASSERT_EQ(dbfull()->GetOptions().compaction_options_universal.stop_style,
            rs::universal_compaction::CompactionStopStyle::kCompactionStopStyleTotalSize);
  ASSERT_EQ(
...
class WriteStallListener : public EventListener {
 public:
  WriteStallListener() : condition_(rs::types::WriteStallCondition::Normal) {}
  void OnStallConditionsChanged(const WriteStallInfo& info) override {
    MutexLock l(&mutex_);
    condition_ = info.condition.cur;
  }
  bool CheckCondition(rs::types::WriteStallCondition expected) {
    MutexLock l(&mutex_);
 ...
 private:
  port::Mutex mutex_;
  rs::types::WriteStallCondition condition_;
};
...
TEST_F(DBTest, SoftLimit) {
  ...
  ASSERT_TRUE(listener->CheckCondition(rs::types::WriteStallCondition::Delayed));
  ...
  ASSERT_TRUE(!dbfull()->TEST_write_controler().NeedsDelay());
  ASSERT_TRUE(listener->CheckCondition(rs::types::WriteStallCondition::Normal));
...
TEST_F(DBTest, ShuttingDownNotBlockStalledWrites) {
  ...
  std::thread thd([&]() {
    ...
    ASSERT_EQ(s.code(), Status::Code::kShutdownInProgress);
  });
