...
Status CheckCFPathsSupported(const DBOptions& db_options,
  ...
  // being used.
  if ((cf_options.compaction_style != rs::advanced_options::CompactionStyle::Universal) &&
      (cf_options.compaction_style != rs::advanced_options::CompactionStyle::Level)) {
...
ColumnFamilyOptions SanitizeOptions(const ImmutableDBOptions& db_options,
  ...
  if (result.compaction_style == rs::advanced_options::CompactionStyle::Level &&
      result.num_levels < 2) {
    result.num_levels = 2;
  }

  if (result.compaction_style == rs::advanced_options::CompactionStyle::Universal &&
  ...
  if (result.compaction_style == rs::advanced_options::CompactionStyle::FIFO) {
  ...
  if (result.ttl == kDefaultTtl) {
    if (is_block_based_table &&
        result.compaction_style != rs::advanced_options::CompactionStyle::FIFO) {
  ...
  // codepath.
  if (result.compaction_style == rs::advanced_options::CompactionStyle::Universal && result.ttl != 0) {
...
ColumnFamilyData::ColumnFamilyData(
  ...
  if (_dummy_versions != nullptr) {
    ...
    if (ioptions_.compaction_style == rs::advanced_options::CompactionStyle::Level) {
      compaction_picker_.reset(
          new LevelCompactionPicker(ioptions_, &internal_comparator_));
    } else if (ioptions_.compaction_style == rs::advanced_options::CompactionStyle::Universal) {
      compaction_picker_.reset(
          new UniversalCompactionPicker(ioptions_, &internal_comparator_));
    } else if (ioptions_.compaction_style == rs::advanced_options::CompactionStyle::FIFO) {
      compaction_picker_.reset(
          new FIFOCompactionPicker(ioptions_, &internal_comparator_));
    } else if (ioptions_.compaction_style == rs::advanced_options::CompactionStyle::None) {
      compaction_picker_.reset(
...
std::pair<rs::types::WriteStallCondition, rs::types::WriteStallCause>
ColumnFamilyData::GetWriteStallConditionAndCause(
  ...
  if (num_unflushed_memtables >= mutable_cf_options.max_write_buffer_number) {
    return {rs::types::WriteStallCondition::Stopped, rs::types::WriteStallCause::MemtableLimit};
  } else if (!mutable_cf_options.disable_auto_compactions &&
             num_l0_files >= mutable_cf_options.level0_stop_writes_trigger) {
    return {rs::types::WriteStallCondition::Stopped, rs::types::WriteStallCause::L0FileCountLimit};
  } else if (!mutable_cf_options.disable_auto_compactions &&
             mutable_cf_options.hard_pending_compaction_bytes_limit > 0 &&
             num_compaction_needed_bytes >=
                 mutable_cf_options.hard_pending_compaction_bytes_limit) {
    return {rs::types::WriteStallCondition::Stopped,
            rs::types::WriteStallCause::PendingCompactionBytes};
  } else if (mutable_cf_options.max_write_buffer_number > 3 &&
             num_unflushed_memtables >=
                 mutable_cf_options.max_write_buffer_number - 1 &&
             num_unflushed_memtables - 1 >=
                 immutable_cf_options.min_write_buffer_number_to_merge) {
    return {rs::types::WriteStallCondition::Delayed, rs::types::WriteStallCause::MemtableLimit};
  } else if (!mutable_cf_options.disable_auto_compactions &&
             mutable_cf_options.level0_slowdown_writes_trigger >= 0 &&
             num_l0_files >=
                 mutable_cf_options.level0_slowdown_writes_trigger) {
    return {rs::types::WriteStallCondition::Delayed, rs::types::WriteStallCause::L0FileCountLimit};
  } else if (!mutable_cf_options.disable_auto_compactions &&
             mutable_cf_options.soft_pending_compaction_bytes_limit > 0 &&
             num_compaction_needed_bytes >=
                 mutable_cf_options.soft_pending_compaction_bytes_limit) {
    return {rs::types::WriteStallCondition::Delayed,
            rs::types::WriteStallCause::PendingCompactionBytes};
  }
  return {rs::types::WriteStallCondition::Normal, rs::types::WriteStallCause::None};
}

rs::types::WriteStallCondition ColumnFamilyData::RecalculateWriteStallConditions(
    const MutableCFOptions& mutable_cf_options) {
  auto write_stall_condition = rs::types::WriteStallCondition::Normal;
  if (current_ != nullptr) {
    ...
    if (write_stall_condition == rs::types::WriteStallCondition::Stopped &&
    ...
    } else if (write_stall_condition == rs::types::WriteStallCondition::Delayed &&
               write_stall_cause == rs::types::WriteStallCause::MemtableLimit) {
      ...
      assert(write_stall_condition == rs::types::WriteStallCondition::Normal);
...
Status ColumnFamilyData::ValidateOptions(
  ...
  if (cf_options.compaction_style == rs::advanced_options::CompactionStyle::FIFO &&
...
Env::WriteLifeTimeHint ColumnFamilyData::CalculateSSTWriteHint(int level) {
  if (initial_cf_options_.compaction_style != rs::advanced_options::CompactionStyle::Level) {
