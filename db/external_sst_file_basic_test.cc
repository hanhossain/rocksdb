...
class ExternalSSTFileBasicTest
 ...
 public:
  ...
  Status GenerateAndAddExternalFile(
      ...
      const std::vector<rs::db::dbformat::ValueType>& value_types,
  ...
  Status GenerateAndAddExternalFile(
      const Options options, std::vector<int> keys,
      const std::vector<rs::db::dbformat::ValueType>& value_types, int file_id,
      bool write_global_seqno, bool verify_checksums_before_ingest,
      std::map<std::string, std::string>* true_data) {
  ...
  Status GenerateAndAddExternalFile(
      const Options options, std::vector<int> keys, const rs::db::dbformat::ValueType value_type,
      int file_id, bool write_global_seqno, bool verify_checksums_before_ingest,
      std::map<std::string, std::string>* true_data) {
    return GenerateAndAddExternalFile(
        options, keys, std::vector<rs::db::dbformat::ValueType>(1, value_type), file_id,
...
TEST_F(ExternalSSTFileBasicTest, IngestFileWithFileChecksum) {
  ...
  ASSERT_EQ(rs::advanced_options::Temperature::Unknown, metadata.levels[6].files[0].temperature);
...
TEST_F(ExternalSSTFileBasicTest, IngestWithTemperature) {
  ...
  rs::advanced_options::Temperature file_temperature = rs::advanced_options::Temperature::Warm;
  ...
  db_->GetColumnFamilyMetaData(&metadata);
  ASSERT_EQ(1, metadata.file_count);
  ASSERT_EQ(rs::advanced_options::Temperature::Warm, metadata.levels[6].files[0].temperature);
  size = GetSstSizeHelper(rs::advanced_options::Temperature::Unknown);
  ASSERT_EQ(size, 0);
  size = GetSstSizeHelper(rs::advanced_options::Temperature::Warm);
  ASSERT_GT(size, 1);
