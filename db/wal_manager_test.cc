...
std::vector<std::uint64_t> ListSpecificFiles(
    Env* env, const std::string& path, const rs::types::FileType expected_file_type) {
  std::vector<std::string> files;
...
TEST_F(WalManagerTest, WALArchivalSizeLimit) {
  ...
  std::vector<std::uint64_t> log_files =
      ListSpecificFiles(env_.get(), archive_dir, rs::types::FileType::WalFile);
  ...
  log_files = ListSpecificFiles(env_.get(), archive_dir, rs::types::FileType::WalFile);
