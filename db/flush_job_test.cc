...
TEST_F(FlushJobTest, NonEmpty) {
  ...
  for (int i = 1; i < 10000; ++i) {
    ...
    ASSERT_OK(new_mem->Add(SequenceNumber(i), rs::db::dbformat::ValueType::TypeValue, key, value,
                           nullptr /* kv_prot_info */));
    if ((i + 1000) % 10000 < 9995) {
      InternalKey internal_key(key, SequenceNumber(i), rs::db::dbformat::ValueType::TypeValue);
      inserted_keys.push_back({internal_key.Encode().ToString(), value});
    }
  }

  {
    ASSERT_OK(new_mem->Add(SequenceNumber(10000), rs::db::dbformat::ValueType::TypeRangeDeletion, "9995",
                           "9999a", nullptr /* kv_prot_info */));
    InternalKey internal_key("9995", SequenceNumber(10000), rs::db::dbformat::ValueType::TypeRangeDeletion);
  ...
  for (size_t i = 0; i < blob_file_numbers.size(); ++i) {
    ...
    ASSERT_OK(new_mem->Add(seq, rs::db::dbformat::ValueType::TypeBlobIndex, key, blob_index,
                           nullptr /* kv_prot_info */));

    InternalKey internal_key(key, seq, rs::db::dbformat::ValueType::TypeBlobIndex);
...
TEST_F(FlushJobTest, FlushMemTablesSingleColumnFamily) {
  ...
  for (size_t i = 0; i != num_mems; ++i) {
    ...
    for (size_t j = 0; j < num_keys_per_table; ++j) {
      ...
      std::string value("value" + key);
      ASSERT_OK(mem->Add(SequenceNumber(j + i * num_keys_per_table), rs::db::dbformat::ValueType::TypeValue,
  ...
  for (auto cfd : all_cfds) {
    ...
    for (size_t i = 0; i != num_memtables[k]; ++i) {
      ...
      for (size_t j = 0; j != num_keys_per_memtable; ++j) {
        ...
        std::string value("value" + key);
        ASSERT_OK(mem->Add(curr_seqno++, rs::db::dbformat::ValueType::TypeValue, key, value,
                           nullptr /* kv_prot_info */));
      }
      mem->ConstructFragmentedRangeTombstones();
      cfd->imm()->Add(mem, &to_delete);
...
TEST_F(FlushJobTest, Snapshots) {
  ...
  for (int i = 1; i < keys; ++i) {
    ...
    for (int j = 0; j < insertions; ++j) {
      ...
      ASSERT_OK(new_mem->Add(SequenceNumber(seqno), rs::db::dbformat::ValueType::TypeValue, key, value,
                             nullptr /* kv_prot_info */));
      ...
      if (visible) {
        InternalKey internal_key(key, seqno, rs::db::dbformat::ValueType::TypeValue);
        inserted_keys.push_back({internal_key.Encode().ToString(), value});
      }
    }
  }
...
TEST_F(FlushJobTimestampTest, AllKeysExpired) {
...

  {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      AddKeyValueToMemtable(new_mem, test::EncodeInt(0), ts, seq,
                            rs::db::dbformat::ValueType::TypeValue, "0_value");
    }
    uint64_t ts = curr_ts_.fetch_add(1);
    SequenceNumber seq = (curr_seq_++);
    AddKeyValueToMemtable(new_mem, test::EncodeInt(0), ts, seq,
...

  {
    ...
    InternalKey ikey(key, curr_seq_ - 1, rs::db::dbformat::ValueType::TypeDeletionWithTimestamp);
...
TEST_F(FlushJobTimestampTest, NoKeyExpired) {
...

  {
    ...
    InternalKey smallest(smallest_key, curr_seq_ - 1, rs::db::dbformat::ValueType::TypeValue);
    InternalKey largest(largest_key, kStartSeq, rs::db::dbformat::ValueType::TypeValue);
