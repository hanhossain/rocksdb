 ...
 protected:
  ...
  void SetColdTemperature(Options& options) {
    if (GetParam()) {
      ...
      options.last_level_temperature = rs::advanced_options::Temperature::Cold;
    }
  }
...
TEST_P(TieredCompactionTest, RangeBasedTieredStorageUniversal) {
  ...
  ASSERT_GT(GetSstSizeHelper(rs::advanced_options::Temperature::Unknown),
            GetSstSizeHelper(rs::advanced_options::Temperature::Cold) * max_size_amp);
...
TEST_F(PrecludeLastLevelTest, PartialPenultimateLevelCompaction) {
  ...
  //  L5: [0,19] [20,39] [40,299]    <-- rs::advanced_options::Temperature::Unknown
  //  L6: [0,19] [20,39]             <-- rs::advanced_options::Temperature::Cold
  ...
  for (const auto& file : meta.levels[5].files) {
    ASSERT_EQ(file.temperature, rs::advanced_options::Temperature::Unknown);
  }
  for (const auto& file : meta.levels[6].files) {
    ASSERT_EQ(file.temperature, rs::advanced_options::Temperature::Cold);
  }
  ASSERT_EQ(meta.levels[6].files[0].smallestkey, Key(0));
