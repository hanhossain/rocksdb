...
extern const rs::db::dbformat::ValueType kValueTypeForSeek;
extern const rs::db::dbformat::ValueType kValueTypeForSeekForPrev;

// Checks whether a type is an inline value type
// (i.e. a type used in memtable skiplist and sst file datablock).
inline bool IsValueType(rs::db::dbformat::ValueType t) {
  return t <= rs::db::dbformat::ValueType::kTypeMerge || rs::db::dbformat::ValueType::kTypeSingleDeletion == t || rs::db::dbformat::ValueType::kTypeBlobIndex == t ||
         rs::db::dbformat::ValueType::kTypeDeletionWithTimestamp == t || rs::db::dbformat::ValueType::kTypeWideColumnEntity == t;
}

// Checks whether a type is from user operation
// ValueType::kTypeRangeDeletion is in meta block so this API is separated from above
// ValueType::kTypeMaxValid can be from keys generated by
// TruncatedRangeDelIterator::start_key()
inline bool IsExtendedValueType(rs::db::dbformat::ValueType t) {
  return IsValueType(t) || t == rs::db::dbformat::ValueType::kTypeRangeDeletion || t == rs::db::dbformat::ValueType::kTypeMaxValid;
}
...
struct ParsedInternalKey {
  ...
  ParsedInternalKey()
      : sequence(kMaxSequenceNumber),
        type(rs::db::dbformat::ValueType::kTypeDeletion)  // Make code analyzer happy
  {}                         // Intentionally left uninitialized (for speed)
  // u contains timestamp if user timestamp feature is enabled.
  ParsedInternalKey(const Slice& u, const SequenceNumber& seq, rs::db::dbformat::ValueType t)
  ...
  void clear() {
    user_key.clear();
    sequence = 0;
    type = rs::db::dbformat::ValueType::kTypeDeletion;
  }
...
inline uint64_t PackSequenceAndType(uint64_t seq, rs::db::dbformat::ValueType t) {
  assert(seq <= kMaxSequenceNumber);
  // ValueType::kTypeMaxValid is used in TruncatedRangeDelIterator, see its constructor.
  assert(IsExtendedValueType(t) || t == rs::db::dbformat::ValueType::kTypeMaxValid);
...
inline void UnPackSequenceAndType(uint64_t packed, uint64_t* seq,
  ...
  *seq = packed >> 8;
  *t = static_cast<rs::db::dbformat::ValueType>(packed & 0xff);
...
inline rs::db::dbformat::ValueType ExtractValueType(const Slice& internal_key) {
  uint64_t num = ExtractInternalKeyFooter(internal_key);
  unsigned char c = num & 0xff;
  return static_cast<rs::db::dbformat::ValueType>(c);
}
...
class InternalKey {
 ...
 public:
  ...
  InternalKey(const Slice& _user_key, SequenceNumber s, rs::db::dbformat::ValueType t) {
    AppendInternalKey(&rep_, ParsedInternalKey(_user_key, s, t));
  }
  InternalKey(const Slice& _user_key, SequenceNumber s, rs::db::dbformat::ValueType t, Slice ts) {
  ...
  void SetMaxPossibleForUserKey(const Slice& _user_key) {
    AppendInternalKey(
        &rep_, ParsedInternalKey(_user_key, 0, static_cast<rs::db::dbformat::ValueType>(0)));
  }
  ...
  void Set(const Slice& _user_key, SequenceNumber s, rs::db::dbformat::ValueType t) {
    SetFrom(ParsedInternalKey(_user_key, s, t));
  }

  void Set(const Slice& _user_key_with_ts, SequenceNumber s, rs::db::dbformat::ValueType t,
           const Slice& ts) {
  ...
  void ConvertFromUserKey(SequenceNumber s, rs::db::dbformat::ValueType t) {
...
inline Status ParseInternalKey(const Slice& internal_key,
  ...
  result->type = static_cast<rs::db::dbformat::ValueType>(c);
  assert(result->type <= rs::db::dbformat::ValueType::kMaxValue);
...
inline void UpdateInternalKey(std::string* ikey, uint64_t seq, rs::db::dbformat::ValueType t) {
  size_t ikey_sz = ikey->size();
  assert(ikey_sz >= kNumInternalBytes);
  uint64_t newval = (seq << 8) | (uint64_t)t;
...
class IterKey {
 public:
  ...
  void UpdateInternalKey(uint64_t seq, rs::db::dbformat::ValueType t, const Slice* ts = nullptr) {
    assert(!IsKeyPinned());
    assert(key_size_ >= kNumInternalBytes);
    if (ts) {
  ...
  void SetInternalKey(const Slice& key_prefix, const Slice& user_key,
                      SequenceNumber s,
                      rs::db::dbformat::ValueType value_type = kValueTypeForSeek,
...
struct RangeTombstone {
  ...
  std::pair<InternalKey, Slice> Serialize() const {
    auto key = InternalKey(start_key_, seq_, rs::db::dbformat::ValueType::kTypeRangeDeletion);
  ...
  InternalKey SerializeKey() const {
    return InternalKey(start_key_, seq_, rs::db::dbformat::ValueType::kTypeRangeDeletion);
  }
  ...
  InternalKey SerializeEndKey() const {
    if (!ts_.empty()) {
      ...
      if (ts_.size() <= strlen(kTsMax)) {
        return InternalKey(end_key_, kMaxSequenceNumber, rs::db::dbformat::ValueType::kTypeRangeDeletion,
