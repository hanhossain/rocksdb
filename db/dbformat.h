...
using ValueType = rs::db::dbformat::ValueType;
...
inline bool IsValueType(ValueType t) {
  return t <= ValueType::kTypeMerge || ValueType::kTypeSingleDeletion == t || ValueType::kTypeBlobIndex == t ||
         ValueType::kTypeDeletionWithTimestamp == t || ValueType::kTypeWideColumnEntity == t;
}
...
inline bool IsExtendedValueType(ValueType t) {
  return IsValueType(t) || t == ValueType::kTypeRangeDeletion || t == ValueType::kTypeMaxValid;
}
...
struct ParsedInternalKey {
  ...
  ParsedInternalKey()
      : sequence(kMaxSequenceNumber),
        type(ValueType::kTypeDeletion)  // Make code analyzer happy
...
inline uint64_t PackSequenceAndType(uint64_t seq, ValueType t) {
  ...
  assert(IsExtendedValueType(t) || t == ValueType::kTypeMaxValid);
...
struct RangeTombstone {
  ...
  std::pair<InternalKey, Slice> Serialize() const {
    auto key = InternalKey(start_key_, seq_, ValueType::kTypeRangeDeletion);
  ...
  InternalKey SerializeKey() const {
    return InternalKey(start_key_, seq_, ValueType::kTypeRangeDeletion);
  }
  ...
  InternalKey SerializeEndKey() const {
    if (!ts_.empty()) {
      ...
      if (ts_.size() <= strlen(kTsMax)) {
        return InternalKey(end_key_, kMaxSequenceNumber, ValueType::kTypeRangeDeletion,
