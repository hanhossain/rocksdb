...
TEST(BlobGarbageMeterTest, MeasureGarbage) {
  ...
  for (const auto& blob : blobs) {
    ...
    const InternalKey key(blob.user_key, seq, rs::db::dbformat::ValueType::kTypeBlobIndex);
...
TEST(BlobGarbageMeterTest, PlainValue) {
  ...
  const InternalKey key(user_key, seq, rs::db::dbformat::ValueType::kTypeValue);
  const Slice key_slice = key.Encode();
...
TEST(BlobGarbageMeterTest, CorruptBlobIndex) {
  ...
  const InternalKey key(user_key, seq, rs::db::dbformat::ValueType::kTypeBlobIndex);
  const Slice key_slice = key.Encode();
