...
class FragmentedRangeTombstoneIterator : public InternalIterator {
 public:
  ...
  ParsedInternalKey parsed_start_key() const {
    return ParsedInternalKey(pos_->start_key, seq(), rs::db::dbformat::ValueType::TypeRangeDeletion);
  }
 ...
 private:
  ...
  void MaybePinKey() const {
    if (pos_ != tombstones_->end() && seq_pos_ != tombstones_->seq_end() &&
      ...
      current_start_key_.Set(pos_->start_key, *seq_pos_, rs::db::dbformat::ValueType::TypeRangeDeletion);
      pinned_pos_ = pos_;
