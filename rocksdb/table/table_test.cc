...
class KeyConvertingIterator : public InternalIterator {
 public:
  ...
  void Seek(const Slice& target) override {
    ParsedInternalKey ikey(target, kMaxSequenceNumber, rs::db::dbformat::ValueType::TypeValue);
    std::string encoded;
 ...
 public:
  ...
  Status FinishImpl(const Options& /*options*/,
    ...
    for (const auto& kv : kv_map) {
      ...
      ParsedInternalKey ikey(kv.first, kMaxSequenceNumber, rs::db::dbformat::ValueType::TypeValue);
      std::string encoded;
      AppendInternalKey(&encoded, ikey);
...
class TableConstructor : public Constructor {
 public:
  ...
  uint64_t ApproximateOffsetOf(const Slice& key) const {
    ...
    if (convert_to_internal_key_) {
      InternalKey ikey(key, kMaxSequenceNumber, rs::db::dbformat::ValueType::TypeValue);
...
class MemTableConstructor : public Constructor {
 public:
  ...
  Status FinishImpl(const Options&, const ImmutableOptions& ioptions,
    ...
    int seq = 1;
    for (const auto& kv : kv_map) {
      Status s = memtable_->Add(seq, rs::db::dbformat::ValueType::TypeValue, kv.first, kv.second,
...
class BlockBasedTableTest
 ...
 public:
  ...
  void SetupTracingTest(TableConstructor* c) {
    ...
    {
      InternalKey internal_key(auto_add_key1, 0, rs::db::dbformat::ValueType::TypeValue);
      std::string encoded_key = internal_key.Encode().ToString();
      c->Add(encoded_key, kDummyValue);
    }
    {
      InternalKey internal_key(auto_add_key2, 0, rs::db::dbformat::ValueType::TypeValue);
      std::string encoded_key = internal_key.Encode().ToString();
      c->Add(encoded_key, kDummyValue);
    }
  }
...
void PrefetchRange(TableConstructor* c, Options* opt,
  ...
  if (key_begin != nullptr) {
    if (c->ConvertToInternalKey()) {
      i_begin.reset(new InternalKey(key_begin, kMaxSequenceNumber, rs::db::dbformat::ValueType::TypeValue));
  ...
  if (key_end != nullptr) {
    if (c->ConvertToInternalKey()) {
      i_end.reset(new InternalKey(key_end, kMaxSequenceNumber, rs::db::dbformat::ValueType::TypeValue));
...
TEST_P(BlockBasedTableTest, TotalOrderSeekOnHashIndex) {
  ...
  for (int i = 0; i <= 4; ++i) {
    ...
    iter->Seek(InternalKey("b", 0, rs::db::dbformat::ValueType::TypeValue).Encode());
    ASSERT_OK(iter->status());
    ASSERT_TRUE(iter->Valid());
    ...
    iter->Seek(InternalKey("bb", 0, rs::db::dbformat::ValueType::TypeValue).Encode());
    ASSERT_OK(iter->status());
    ASSERT_TRUE(iter->Valid());
    ASSERT_EQ("bbaa1", ExtractUserKey(iter->key()).ToString());
    iter->Next();
    ...
    iter->Seek(InternalKey("bbb", 0, rs::db::dbformat::ValueType::TypeValue).Encode());
    ASSERT_OK(iter->status());
    ASSERT_TRUE(iter->Valid());
    ASSERT_EQ("bbbb1", ExtractUserKey(iter->key()).ToString());
    iter->Next();
...
TEST_P(BlockBasedTableTest, NoopTransformSeek) {
  ...
  // the user-key.
  InternalKey key("a", 1, rs::db::dbformat::ValueType::TypeValue);
...
TEST_P(BlockBasedTableTest, SkipPrefixBloomFilter) {
  ...
  TableConstructor c(options.comparator);
  InternalKey key("abcdefghijk", 1, rs::db::dbformat::ValueType::TypeValue);
...
TEST_P(BlockBasedTableTest, BadChecksumType) {
  ...
  TableConstructor c(options.comparator);
  InternalKey key("abc", 1, rs::db::dbformat::ValueType::TypeValue);
  c.Add(key.Encode().ToString(), "test");
  std::vector<std::string> keys;
  stl_wrappers::KVMap kvmap;
  const ImmutableOptions ioptions(options);
...
void AddInternalKey(TableConstructor* c, const std::string& prefix,
  ...
  InternalKey k(prefix + rnd.RandomString(800), 0, rs::db::dbformat::ValueType::TypeValue);
...
void TableTest::IndexTest(BlockBasedTableOptions table_options) {
  ...
  for (size_t i = 0; i < prefixes.size(); ++i) {
    index_iter->Seek(InternalKey(prefixes[i], 0, rs::db::dbformat::ValueType::TypeValue).Encode());
  ...
  for (size_t i = 0; i < prefixes.size(); ++i) {
    ...
    index_iter->Seek(InternalKey(key, 0, rs::db::dbformat::ValueType::TypeValue).Encode());
  ...
  for (const auto& prefix : non_exist_prefixes) {
    index_iter->Seek(InternalKey(prefix, 0, rs::db::dbformat::ValueType::TypeValue).Encode());
  ...
  for (const auto& prefix : non_exist_prefixes) {
    index_iter->SeekForPrev(InternalKey(prefix, 0, rs::db::dbformat::ValueType::TypeValue).Encode());
    // regular_iter->Seek(prefix);
...

  {
    ...
    index_iter2->Seek(InternalKey("0055", 0, rs::db::dbformat::ValueType::TypeValue).Encode());
    ...
    index_iter2->Seek(InternalKey("0095", 0, rs::db::dbformat::ValueType::TypeValue).Encode());
    ASSERT_TRUE(index_iter2->Valid());
    ASSERT_EQ("0095", index_iter2->key().ToString().substr(0, 4));
    index_iter2->Prev();
    ASSERT_TRUE(index_iter2->Valid());
    ...
    index_iter2->Seek(InternalKey("0075", 0, rs::db::dbformat::ValueType::TypeValue).Encode());
    ASSERT_TRUE(index_iter2->Valid());
    ASSERT_EQ("0075", index_iter2->key().ToString().substr(0, 4));
...
TEST_P(BlockBasedTableTest, IndexSeekOptimizationIncomplete) {
  ...
  auto ikey = [](Slice user_key) {
    return InternalKey(user_key, 0, rs::db::dbformat::ValueType::TypeValue).Encode().ToString();
  };
...
TEST_P(BlockBasedTableTest, BinaryIndexWithFirstKeyGlobalSeqno) {
  ...
  c.Add(InternalKey("b", 0, rs::db::dbformat::ValueType::TypeValue).Encode().ToString(), "x");
  c.Add(InternalKey("c", 0, rs::db::dbformat::ValueType::TypeValue).Encode().ToString(), "y");
  ...
  iter->Seek(InternalKey("a", 0, rs::db::dbformat::ValueType::TypeValue).Encode().ToString());
  ASSERT_TRUE(iter->Valid());
  EXPECT_EQ(InternalKey("b", 42, rs::db::dbformat::ValueType::TypeValue).Encode().ToString(),
            iter->key().ToString());
...
TEST_P(BlockBasedTableTest, TracingMultiGetTest) {
  ...
  for (bool first_pass : {true, false}) {
    ...
    encoded_keys[0] = InternalKey(ukeys[0], 0, rs::db::dbformat::ValueType::TypeValue).Encode().ToString();
    encoded_keys[1] = InternalKey(ukeys[1], 0, rs::db::dbformat::ValueType::TypeValue).Encode().ToString();
...
TEST_P(BlockBasedTableTest, FilterBlockInBlockCache) {
  ...
  InternalKey internal_key(user_key, 0, rs::db::dbformat::ValueType::TypeValue);
...
TEST_P(BlockBasedTableTest, BlockReadCountTest) {
  ...
  for (int bloom_filter_type = 1; bloom_filter_type <= 2; ++bloom_filter_type) {
    for (int index_and_filter_in_cache = 0; index_and_filter_in_cache < 2;
      ...
      internal_key = InternalKey(user_key, 0, rs::db::dbformat::ValueType::TypeValue);
...
TEST_P(IndexBlockRestartIntervalTest, IndexBlockRestartInterval) {
  ...
  for (int i = 0; i < kKeysInTable; i++) {
    InternalKey k(rnd.RandomString(kKeySize), 0, rs::db::dbformat::ValueType::TypeValue);
...
TEST_P(BlockBasedTableTest, DISABLED_TableWithGlobalSeqno) {
  ...
  for (char c = 'a'; c <= 'z'; ++c) {
    ...
    InternalKey ik(key, 0, rs::db::dbformat::ValueType::TypeValue);
  ...
  for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
    ...
    ASSERT_EQ(pik.type, rs::db::dbformat::ValueType::TypeValue);
...
TEST_P(BlockBasedTableTest, DataBlockHashIndex) {
  ...
  for (int i = 0; i < kNumKeys; i++) {
    ...
    InternalKey k(random_key, 0, rs::db::dbformat::ValueType::TypeValue);
    c.Add(k.Encode().ToString(), rnd.RandomString(kValSize));
  }
...
TEST_F(ChargeCompressionDictionaryBuildingBufferTest, Basic) {
  ...
  for (CacheEntryRoleOptions::Decision
    ...
    InternalKey ik1(key1, 0 /* sequnce number */, rs::db::dbformat::ValueType::TypeValue);
    ...
    InternalKey ik2(key2, 1 /* sequnce number */, rs::db::dbformat::ValueType::TypeValue);
...
TEST_F(ChargeCompressionDictionaryBuildingBufferTest,
  ...
  InternalKey ik3(key3, 2 /* sequnce number */, rs::db::dbformat::ValueType::TypeValue);
