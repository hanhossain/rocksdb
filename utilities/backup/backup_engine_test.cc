 ...
 public:
  ...
  Status GetLiveFilesStorageInfo(
    ...
    for (auto& f : live_files_) {
      ...
      } else if (type == rs::types::FileType::CurrentFile) {
        info.size = 0;
        info.trim_to_size = true;
      } else {
...
class BackupEngineTest : public testing::Test {
 public:
  ...
  Status GetDataFilesInDB(const rs::types::FileType& file_type,
  ...
  Status GetRandomDataFileInDB(const rs::types::FileType& file_type,
  ...
  Status CorruptRandomDataFileInDB(const rs::types::FileType& file_type) {
    std::string fname;
    uint64_t fsize = 0;
...
TEST_F(BackupEngineTest, TableFileCorruptedBeforeBackup) {
  ...
  ASSERT_OK(CorruptRandomDataFileInDB(rs::types::FileType::TableFile));
...
TEST_F(BackupEngineTest, BlobFileCorruptedBeforeBackup) {
  ...
  ASSERT_OK(CorruptRandomDataFileInDB(rs::types::FileType::BlobFile));
...
TEST_F(BackupEngineTest, TableFileCorruptionBeforeIncremental) {
  ...
  for (bool corrupt_before_first_backup : {false, true}) {
    for (ShareFilesNaming option :
      ...
      ASSERT_OK(GetDataFilesInDB(rs::types::FileType::TableFile, &table_files));
...
TEST_F(BackupEngineTest, BackgroundThreadCpuPriority) {
  std::atomic<rs::port_defs::CpuPriority> priority(rs::port_defs::CpuPriority::Normal);
  ROCKSDB_NAMESPACE::SyncPoint::GetInstance()->SetCallBack(
...

  {
    ...
    ASSERT_EQ(priority, rs::port_defs::CpuPriority::Normal);
  }

  {
    ...
    options.background_thread_cpu_priority = rs::port_defs::CpuPriority::Low;
    ASSERT_OK(backup_engine_->CreateNewBackup(options, db_.get()));

    ASSERT_EQ(priority, rs::port_defs::CpuPriority::Low);
  }

  {
    ...
    CreateBackupOptions options;
    options.decrease_background_thread_cpu_priority = true;
    options.background_thread_cpu_priority = rs::port_defs::CpuPriority::Normal;
    ASSERT_OK(backup_engine_->CreateNewBackup(options, db_.get()));
...

  {
    ...
    CreateBackupOptions options;
    options.decrease_background_thread_cpu_priority = true;
    options.background_thread_cpu_priority = rs::port_defs::CpuPriority::Idle;
    ASSERT_OK(backup_engine_->CreateNewBackup(options, db_.get()));

    ASSERT_EQ(priority, rs::port_defs::CpuPriority::Idle);
  }

  {
    ...
    priority = rs::port_defs::CpuPriority::Normal;
...
TEST_F(BackupEngineTest, FileTemperatures) {
  ...
  std::map<rs::advanced_options::Temperature, int> manifest_temp_counts;
  {
    ...
    for (auto info : infos) {
      if (info.file_type == rs::types::FileType::TableFile) {
