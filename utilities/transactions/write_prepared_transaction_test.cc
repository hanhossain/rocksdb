 ...
 protected:
  ...
  void VerifyInternalKeys(const std::vector<KeyVersion>& expected_versions) {
    ...
    for (size_t i = 0; i < versions.size(); i++) {
      ...
      if (static_cast<ValueType>(versions[i].type) != ValueType::kTypeDeletion &&
          static_cast<ValueType>(versions[i].type) != ValueType::kTypeSingleDeletion) {
        ASSERT_EQ(expected_versions[i].value, versions[i].value);
      }
      // Range delete not supported.
      ASSERT_NE(expected_versions[i].type, ValueType::kTypeRangeDeletion);
    }
  }
};
...
TEST_P(WritePreparedTransactionTest, DisableGCDuringRecovery) {
  ...
  for (uint64_t i = 1; i <= 1024; i++) {
    ...
    KeyVersion kv = {"foo", v, seq, ValueType::kTypeValue};
    if (options.two_write_queues) {
...
TEST_P(WritePreparedTransactionTest, SequenceNumberZero) {
  ...
  VerifyInternalKeys({{"foo", "bar", 0, ValueType::kTypeValue}});
...
TEST_P(WritePreparedTransactionTest, CompactionShouldKeepUncommittedKeys) {
  ...
  VerifyInternalKeys({
      {"key1", "value1_2", expected_seq, ValueType::kTypeValue},
      {"key1", "value1_1", 0, ValueType::kTypeValue},
      {"key2", "", expected_seq, ValueType::kTypeDeletion},
      {"key2", "value2_1", 0, ValueType::kTypeValue},
      {"key3", "", expected_seq, ValueType::kTypeSingleDeletion},
      {"key3", "value3_1", 0, ValueType::kTypeValue},
      {"key4", "value4_2", expected_seq, ValueType::kTypeMerge},
      {"key4", "value4_1", 0, ValueType::kTypeValue},
      {"key5", "value5_3", expected_seq, ValueType::kTypeMerge},
      {"key5", "value5_1,value5_2", 0, ValueType::kTypeValue},
      {"key6", "value6_2", expected_seq, ValueType::kTypeValue},
      {"key7", "value7_2", expected_seq, ValueType::kTypeValue},
  });
...
TEST_P(WritePreparedTransactionTest, ReleaseSnapshotDuringCompaction2) {
  ...
  VerifyInternalKeys({{"key1", "value2", v2_seq, ValueType::kTypeValue}});
...
TEST_P(WritePreparedTransactionTest, ReleaseEarliestSnapshotDuringCompaction) {
  ...
  VerifyInternalKeys({{"key1", "", del_seq, ValueType::kTypeDeletion},
                      {"key1", "value1", put_seq, ValueType::kTypeValue}});
  db->ReleaseSnapshot(snapshot2);
  SyncPoint::GetInstance()->ClearAllCallBacks();
}
...
TEST_P(WritePreparedTransactionTest,
  ...
  VerifyInternalKeys({
      ...
      {"key1", "value1", seq1, ValueType::kTypeValue},
      // "key2" is committed and output with seq = 0.
      {"key2", "value2", 0, ValueType::kTypeValue},
  });
  ASSERT_OK(transaction->Commit());
  VerifyKeys({
      {"key1", "value1"},
