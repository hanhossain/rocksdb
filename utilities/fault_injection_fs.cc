...
TestFSWritableFile::~TestFSWritableFile() {
  if (writable_file_opened_) {
    Close(IOOptions(), nullptr).inner_status.PermitUncheckedError();
  }
}
...
IOStatus TestFSWritableFile::Append(const Slice& data, const IOOptions& options,
  ...
  if (target_->use_direct_io()) {
    target_->Append(data, options, dbg).inner_status.PermitUncheckedError();
  } else {
...
IOStatus TestFSWritableFile::Close(const IOOptions& options,
  ...
  if (io_s.inner_status.ok()) {
    ...
    // Ignore sync errors
    target_->Sync(options, dbg).inner_status.PermitUncheckedError();
...
IOStatus TestFSWritableFile::RangeSync(uint64_t offset, uint64_t nbytes,
  ...
  // Ignore sync errors
  target_->RangeSync(offset, nbytes, options, dbg).inner_status.PermitUncheckedError();
...
IOStatus TestFSRandomAccessFile::Read(uint64_t offset, size_t n,
  ...
  if (s.inner_status.ok() && fs_->ShouldInjectRandomReadError()) {
...
IOStatus TestFSRandomAccessFile::MultiRead(FSReadRequest* reqs, size_t num_reqs,
  ...
  for (size_t i = 0; i < num_reqs; i++) {
    if (!reqs[i].status.inner_status.ok()) {
...
IOStatus FaultInjectionTestFS::ReopenWritableFile(
  ...
  if (exists_s.inner_status.IsNotFound()) {
    exists = false;
  } else if (exists_s.inner_status.ok()) {
    exists = true;
  } else {
    io_s = exists_s;
    exists = false;
...
IOStatus FaultInjectionTestFS::RenameFile(const std::string& s,
  ...
  if (target()->FileExists(t, IOOptions(), nullptr).inner_status.ok()) {
    uint64_t file_size;
    if (target()->GetFileSize(t, IOOptions(), &file_size, nullptr).inner_status.ok() &&
        file_size < 1024) {
      ReadFileToString(target(), t, &previous_contents).inner_status.PermitUncheckedError();
    }
  }
...
IOStatus FaultInjectionTestFS::DropUnsyncedFileData() {
  ...
  for (std::map<std::string, FSFileState>::iterator it = db_file_state_.begin();
       io_s.inner_status.ok() && it != db_file_state_.end(); ++it) {
...
bool FaultInjectionTestFS::TryParseFileName(const std::string& file_name,
                                            uint64_t* number, rs::types::FileType* type) {
...
IOStatus FaultInjectionTestFS::InjectWriteError(const std::string& file_name) {
  ...
  if (inject_for_all_file_types_) {
    ...
    uint64_t number;
    rs::types::FileType cur_type = rs::types::FileType::TempFile;
