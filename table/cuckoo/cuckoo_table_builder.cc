...
void CuckooTableBuilder::Add(const Slice& key, const Slice& value) {
  ...
  if (ikey.type != rs::db::dbformat::ValueType::TypeDeletion && ikey.type != rs::db::dbformat::ValueType::TypeValue) {
...
Status CuckooTableBuilder::Finish() {
  ...
  if (num_entries_ > 0) {
    ...
    if (is_last_level_file_) {
      ...
      ParsedInternalKey ikey(unused_user_key, 0, rs::db::dbformat::ValueType::TypeValue);
