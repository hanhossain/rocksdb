...
TEST_F(CuckooReaderTest, WhenKeyExists) {
  ...
  for (uint64_t i = 0; i < num_items; i++) {
    ...
    ParsedInternalKey ikey(user_keys[i], i + 1000, rs::db::dbformat::ValueType::TypeValue);
    AppendInternalKey(&keys[i], ikey);
...
TEST_F(CuckooReaderTest, CheckIterator) {
  ...
  for (uint64_t i = 0; i < num_items; i++) {
    user_keys[i] = "key" + NumToStr(i);
    ParsedInternalKey ikey(user_keys[i], 1000, rs::db::dbformat::ValueType::TypeValue);
    AppendInternalKey(&keys[i], ikey);
    values[i] = "value" + NumToStr(i);
...
TEST_F(CuckooReaderTest, WhenKeyNotFound) {
  ...
  ParsedInternalKey ikey(not_found_user_key, 1000, rs::db::dbformat::ValueType::TypeValue);
  ...
  ParsedInternalKey ikey2(not_found_user_key2, 1000, rs::db::dbformat::ValueType::TypeValue);
...
void GetKeys(uint64_t num, std::vector<std::string>* keys) {
  keys->clear();
  IterKey k;
  k.SetInternalKey("", 0, rs::db::dbformat::ValueType::TypeValue);
