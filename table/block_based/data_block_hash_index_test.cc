...
TEST(DataBlockHashIndex, BlockTestSingleKey) {
  ...
  InternalKey ikey(ukey, 10, ValueType::kTypeValue);
...
TEST(DataBlockHashIndex, BlockBoundary) {
...

  {
    ...
    // seek for "axy"@60
    std::string uk1("aab");
    InternalKey ik1(uk1, 100, ValueType::kTypeValue);
    std::string v1(4100, '1');  // large value

    std::string uk2("axy");
    InternalKey ik2(uk2, 10, ValueType::kTypeValue);
    std::string v2(4100, '2');  // large value

    PinnableSlice value;
    std::string seek_ukey("axy");
    InternalKey seek_ikey(seek_ukey, 60, ValueType::kTypeValue);
...

  {
    ...
    PinnableSlice value;
    std::string seek_ukey("axy");
    InternalKey seek_ikey(seek_ukey, 120, ValueType::kTypeValue);
    GetContext get_context(options.comparator, nullptr, nullptr, nullptr,
                           GetContext::kNotFound, seek_ukey, &value, nullptr,
                           nullptr, nullptr, true, nullptr, nullptr);
...

  {
    ...
    PinnableSlice value;
    std::string seek_ukey("axy");
    InternalKey seek_ikey(seek_ukey, 5, ValueType::kTypeValue);
    GetContext get_context(options.comparator, nullptr, nullptr, nullptr,
                           GetContext::kNotFound, seek_ukey, &value, nullptr,
                           nullptr, nullptr, true, nullptr, nullptr);
