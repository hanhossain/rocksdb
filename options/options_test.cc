...
TEST_F(OptionsTest, GetOptionsFromMapTest) {
  ...
  exact.sanity_level = rs::convenience::SanityLevel::ExactMatch;
  loose.sanity_level = rs::convenience::SanityLevel::LooselyCompatible;
  ...
  ASSERT_EQ(new_cf_opt.last_level_temperature, rs::advanced_options::Temperature::Warm);
  ASSERT_EQ(new_cf_opt.bottommost_temperature, rs::advanced_options::Temperature::Warm);
...
TEST_F(OptionsParserTest, DifferentDefault) {
  ...
  ASSERT_EQ(rs::advanced_options::CompactionPri::MinOverlappingRatio, cf_level_opts.compaction_pri);
...

  {
    ...
    ASSERT_EQ(rs::options::WALRecoveryMode::TolerateCorruptedTailRecords,
  ...
  }
  {
    ...
    ASSERT_EQ(rs::advanced_options::CompactionPri::ByCompensatedSize,
  ...
  }
  {
    ...
    ASSERT_TRUE(old_default_opts.compaction_pri ==
                rs::advanced_options::CompactionPri::ByCompensatedSize);
  }
  {
    Options old_default_opts;
...
class OptionsSanityCheckTest : public OptionsParserTest,
 ...
 protected:
  Status SanityCheckOptions(const DBOptions& db_opts,
                            ...
                            rs::convenience::SanityLevel level) {
  ...
  void SanityCheckCFOptions(const ColumnFamilyOptions& opts, bool exact) {
    ASSERT_OK(SanityCheckCFOptions(
        opts, rs::convenience::SanityLevel::LooselyCompatible));
    ASSERT_OK(SanityCheckCFOptions(opts, rs::convenience::SanityLevel::None));
    if (exact) {
      ASSERT_OK(
          SanityCheckCFOptions(opts, rs::convenience::SanityLevel::ExactMatch));
    } else {
      ASSERT_NOK(
          SanityCheckCFOptions(opts, rs::convenience::SanityLevel::ExactMatch));
    }
  ...
  void SanityCheckDBOptions(const DBOptions& opts, bool exact) {
    ASSERT_OK(SanityCheckDBOptions(
        opts, rs::convenience::SanityLevel::LooselyCompatible));
    ASSERT_OK(SanityCheckDBOptions(opts, rs::convenience::SanityLevel::None));
    if (exact) {
      ASSERT_OK(
          SanityCheckDBOptions(opts, rs::convenience::SanityLevel::ExactMatch));
    } else {
      ASSERT_NOK(
          SanityCheckDBOptions(opts, rs::convenience::SanityLevel::ExactMatch));
    }
...
TEST_P(OptionsSanityCheckTest, MergeOperatorErrorMessage) {
  ...
  Status s =
      SanityCheckCFOptions(opts, rs::convenience::SanityLevel::LooselyCompatible);
  ASSERT_TRUE(s.IsInvalidArgument());
  ...
  opts.merge_operator.reset(test::RandomMergeOperator(&rnd));
  s = SanityCheckCFOptions(opts, rs::convenience::SanityLevel::LooselyCompatible);
  ASSERT_TRUE(s.IsInvalidArgument());
  err_msg = s.ToString();
  specified =
...
TEST_P(OptionsSanityCheckTest, CFOptionsSanityCheck) {
  ...
  // persist the change
  ASSERT_OK(PersistCFOptions(opts));
  ASSERT_OK(SanityCheckCFOptions(opts, rs::convenience::SanityLevel::ExactMatch));
...
TEST_P(OptionsSanityCheckTest, DBOptionsSanityCheck) {
  ...
  // persist the change
  ASSERT_OK(PersistDBOptions(opts));
  ASSERT_OK(SanityCheckDBOptions(opts, rs::convenience::SanityLevel::ExactMatch));
}
...
TEST_F(OptionTypeInfoTest, TestEqualsFunc) {
  OptionTypeInfo opt_info(0, rs::options_type::OptionType::Int, rs::options_type::OptionVerificationType::Normal,
                          rs::options_type::OptionTypeFlags::None);
...
TEST_F(OptionTypeInfoTest, TestOptionFlags) {
  ...
  OptionTypeInfo opt_never(0, rs::options_type::OptionType::String,
                           rs::options_type::OptionVerificationType::Normal,
                           rs::options_type::OptionTypeFlags::CompareNever);
  OptionTypeInfo opt_alias(0, rs::options_type::OptionType::String,
                           rs::options_type::OptionVerificationType::Alias,
                           rs::options_type::OptionTypeFlags::None);
  OptionTypeInfo opt_deprecated(0, rs::options_type::OptionType::String,
                                rs::options_type::OptionVerificationType::Deprecated,
                                rs::options_type::OptionTypeFlags::None);
  ConfigOptions config_options;
  std::string opts_str;
...
TEST_F(OptionTypeInfoTest, TestStruct) {
  ...
  OptionTypeInfo basic_info = OptionTypeInfo::Struct(
      "b", &basic_type_map, 0, rs::options_type::OptionVerificationType::Normal,
  ...
  OptionTypeInfo extended_info = OptionTypeInfo::Struct(
      "e", &extended_type_map, 0, rs::options_type::OptionVerificationType::Normal,
      rs::options_type::OptionTypeFlags::Mutable);
  Extended e1, e2;
  ConfigOptions config_options;
  std::string mismatch;
